...
fn rasterize(
    ...
    let screen = Face3::from_iterator(ndc.iter().map(|vertex| {
        Vec3::new(
            (vertex.x * x_scale) + x_translate,
            (vertex.y * y_scale) + y_translate,
            vertex.z,
        )
    }));
...
pub fn draw_frame(
    ...
    *framebuffer = Frame::from_element(options.viewport_width, options.viewport_height, 0xFF16110E);
